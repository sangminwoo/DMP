...
class GaussianDiffusion:
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if model.model.module.gate_type == "FT":
                model_output = model.model.module.original_forward(x_t, t, **model_kwargs)
            else:
        ...
        elif self.loss_type == LossType.MSE_gate:
            model_output = model(x_t, t, **model_kwargs)
            ...
            if self.model_var_type in [
                ...
                assert model_output.shape == (B, C * 2, *x_t.shape[2:])
                model_output, model_var_values = th.split(model_output, C, dim=1)
                # model_output_ref, _ = th.split(model_output_ref, C, dim=1)
                # Learn the variance using the variational bound, but don't let
                # it affect our mean prediction.
                frozen_out = th.cat([model_output.detach(), model_var_values], dim=1)
                terms["vb"] = self._vb_terms_bpd(
            ...
            gates = th.stack(model.model.module.routing_mask, dim=0)
