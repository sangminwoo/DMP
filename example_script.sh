...
# Train Parameters
DATA_PATH="ImageNet/train/" # default is null, so you have to set the data_path or errors will raise up!
PRETRAINED_PATH=""

iterations=50000

# Sample Parameters
num_fid_samples=200 # 10000, 50000 (commonly used)
cfg_scale=1.5 # 1.0 1.5 (commonly used)
version=0 # default : 0, have to set if you have multiple versions
eval_at=20000 # default : train iteration, have to set this for iteration-wise evaluation


# Train Script
torchrun --nnodes=1 --nproc_per_node=1 --master_port 8772 train.py \
data=${dataset} \
...
#torchrun --nnodes=1 --nproc_per_node=1 --master_port 1552 sample_ddp.py \
#data=${dataset} \
#general.schedule_name=${scheduling} \
#general.loss_weight_type=${loss_weight_type} \
#models.name=${model_name} \
